...
if (isset($orderDetails)){
    ...
    $details = '<div class="col-sm-6" style="text-align: left; font-size: medium">
            <label><strong>Order name: </strong>'. $orderDetails->order_name .'</label><br>
            <label><strong>Date Placed: </strong>'. $orderDetails->order_date.'</label><br>
            <label><strong>Expected Date: </strong>'. $orderDetails->expected_date.'</label><br>
            <label><strong>Customer Name: </strong>'. $orderDetails->full_name.'</label><br>
            <label><strong>Email Address: </strong>'. $orderDetails->email.'</label><br>
            <label><strong>Phone Number: </strong>'. $orderDetails->phone.'</label><br>
            <label><strong>Customer Location: </strong>'. $orderDetails->location.'</label><br>
    ...
    $table2.= '<tr style="background-color: #00A8B3">
                ...
                <th style="border: solid 1px #BDBDBD">Features</th>
            
           </tr>';
    $table2 .= '</thead>';
    $num = 1;

    foreach ($singleOrder->result() as $order){
        $table2 .='<tr style="padding: ">
            <td style="border: solid 1px #BDBDBD; padding: 8px">'.$num++. '</td>
            <td style="border: solid 1px #BDBDBD; padding: 8px">'.$order->orderedProduct_name. '</td>
            <td style="border: solid 1px #BDBDBD; padding: 8px">'.$order->orderedProduct_quantity. '</td>
            <td style="border: solid 1px #BDBDBD; padding: 8px">'.$order->orderedProduct_features. '</td>
        </tr>';
    }
...
foreach ($cartProduct->result() as $row){
